...
const Editor = () => {
    ...
    // Auto-fit preview scaling based on container width
    useEffect(() => {
    ...
    const paddingCompensation = containerWidth < 640 ? 8 : 24;
    const baseScale = containerWidth ? Math.min((containerWidth - paddingCompensation) / 794, 1.2) : 0.5;
    const activeZoom = (zoom / 100) * baseScale;
    ...
    return (
        <div className="h-screen bg-background flex flex-col overflow-hidden text-text selection:bg-primary/20">
            ...
            <div className="h-14 border-b border-border bg-background px-4 sm:px-6 flex items-center justify-between flex-shrink-0 z-30">
                <div className="flex items-center gap-3 sm:gap-6">
                    ...
                    <div className="flex flex-col">
                        ...
                        <input
                            className="bg-transparent border-none p-0 text-sm font-bold text-heading focus:ring-0 w-24 sm:w-48 hover:bg-surface-highlight rounded px-1 transition-colors outline-none cursor-text"
                            value={localTitle}
                            onChange={(e) => {
                    ...
                    {isSaving ? (
                        <div className="hidden lg:flex items-center gap-2 text-[10px] font-bold text-primary uppercase tracking-widest bg-primary/5 px-3 py-1 rounded-full border border-primary/10">
                            <div className="w-1.5 h-1.5 rounded-full bg-primary animate-pulse" />
                            Saving to cloud
                        </div>
                    ) : (
                        <div className="hidden lg:flex items-center gap-2 text-[10px] font-bold text-subtext uppercase tracking-widest bg-surface-highlight px-3 py-1 rounded-full border border-border">
                    ...
                    <button
                        ...
                        className="flex items-center gap-2 text-[10px] font-bold text-heading uppercase tracking-widest bg-background hover:bg-surface-highlight px-2 sm:px-3 py-1.5 rounded-lg border border-border transition-all ml-1 sm:ml-2"
                ...
                <div className="flex items-center gap-2 sm:gap-4">
                    ...
                    <button
                        ...
                        className="btn-primary py-2 px-3 sm:px-6 text-xs font-bold shadow-lg shadow-primary/20 whitespace-nowrap"
                    >
                        <Download size={16} /> <span className="hidden sm:inline">Export Resume</span><span className="sm:hidden">Export</span>
                    </button>
                </div>
            </div>
                                ...
                                >
                                    <div className="accordion-card-content p-3 sm:p-6 border-t border-border pt-4 bg-background">
                                        <div className="flex justify-end mb-4">
                                            ...
                                            >
                                                <RotateCcw size={10} className="sm:w-3 sm:h-3" /> <span>Reset Layout</span>
                                            </button>
                                        </div>
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-8">
                                            <div className="space-y-3">
                                                <div className="flex justify-between items-center">
                                                    <label className="text-[8px] sm:text-[10px] font-black text-zinc-500 uppercase tracking-widest">Section Gap</label>
                                                    <span className="text-[8px] sm:text-[10px] font-mono font-black bg-zinc-100 dark:bg-zinc-700 px-1.5 py-0.5 sm:px-2 sm:py-1 rounded text-zinc-900 dark:text-zinc-200">
                                        ...
                                        {(exp.description || []).map((bullet, bIndex) => (
                                            <div key={bIndex} className="flex gap-2 group/bullet">
                                                ...
                                                <button
                                                    ...
                                                    className="p-2 text-subtext hover:text-red-500 opacity-100 sm:opacity-0 sm:group-hover/bullet:opacity-100"
                                                >
                                                    <X size={14} />
                                                </button>
                                            </div>
                ...
                <section className={`border-l border-border relative ${mobileView === 'preview' ? 'flex flex-1 w-full h-full flex-col' : 'hidden sm:flex sm:flex-1 sm:w-1/2 sm:h-full sm:flex-col'}`}>
                    ...
                    <div className="absolute bottom-3 right-3 sm:top-6 sm:right-6 sm:bottom-auto z-40 flex flex-row sm:flex-col gap-1.5 sm:gap-2 bg-background/95 dark:bg-zinc-800/95 sm:bg-transparent backdrop-blur border border-border/60 sm:border-none p-1 sm:p-0 rounded-full sm:rounded-none shadow-lg sm:shadow-none items-center">
                        <div className="flex items-center gap-1 bg-transparent sm:bg-background sm:border sm:border-border p-0 sm:p-1 rounded-full">
                            ...
                            <div className="w-8 sm:w-12 text-center text-[8px] sm:text-[10px] font-black text-heading">{zoom}%</div>
                            <button onClick={handleZoomIn} className="p-1 sm:p-2 hover:bg-surface-highlight rounded-full text-subtext" title="Zoom In">
                                <ZoomIn size={12} className="sm:w-4 sm:h-4" />
                            </button>
                        </div>
                        <button onClick={() => setZoom(100)} className="flex items-center justify-center p-1 sm:p-2 bg-background border border-border sm:border-border rounded-full text-subtext hover:text-heading shadow-md sm:shadow-none" title="Reset Zoom">
                    ...
                    >
                        <div 
                            ...
                            className="transition-all duration-300 ease-out shadow-[0_32px_64px_-16px_rgba(0,0,0,0.2)]"
                        >
                            <div
